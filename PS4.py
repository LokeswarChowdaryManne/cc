...
# Euler Table with matplotlib
data = []
for i, (x, y_num) in enumerate(zip(x_euler, y_euler)):
    y_true = 2 * np.exp(x) - x - 1
    error = abs(y_num - y_true)
    data.append([i, round(x, 4), round(y_num, 6), round(y_true, 6), round(error, 6)])
...
# RK4 Table with matplotlib
data = []
for i, (x, y_num, y_true) in enumerate(zip(x_rk4, y_rk4, y_exact_at_rk4)):
    error = abs(y_num - y_true)
    rel_error = 100 * error / y_true if y_true != 0 else float('nan')
    data.append([i, round(x, 4), round(y_num, 6), round(y_true, 6), round(error, 6), round(rel_error, 4)])

fig, ax = plt.subplots(figsize=(12, 5))
ax.axis('tight')
ax.axis('off')
columns = ["Step", "x", "RK4 (y)", "Exact (y)", "Error", "Relative Error (%)"]
table = ax.table(cellText=data, colLabels=columns, cellLoc='center', loc='center')
table.auto_set_font_size(False)
table.set_fontsize(10)
table.scale(1.2, 1.2)
